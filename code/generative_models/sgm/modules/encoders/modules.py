...
class FrozenCLIPEmbedder(AbstractEmbModel):
    ...
    def __init__(
        ...
        try:
            ...
            print(f"Warning: Failed to load {version} from transformers. Falling back to OpenAI CLIP library.")
            import clip
        ...
        if self.use_clip_fallback:
            ...
            with torch.no_grad():
                ...
                if self.layer == "last":
                    ...
                    z = x[torch.arange(x.shape[0]), tokens.argmax(dim=-1)] @ self.model.text_projection
                    z = z[:, None, :]
                else:
                    # Fallback for hidden states if needed, might not be exact match
                     print("Warning: accessing hidden states in CLIP fallback not fully supported")
                     z = x
                
                if self.return_pooled:
                     p = x[torch.arange(x.shape[0]), tokens.argmax(dim=-1)] @ self.model.text_projection
                     return z, p
                return z
...
class FrozenOpenCLIPEmbedder2(AbstractEmbModel):
    ...
    def __init__(
        ...
        try:
            ...
            print(f"Warning: Failed to load OpenCLIP {version}. Trying local fallback.")
            import os
            fallback_path = "/home/liujiaxiang/MindAligner/models_weights/open_clip_pytorch_model.bin"
            if os.path.exists(fallback_path):
                model, _, _ = open_clip.create_model_and_transforms(
                    arch,
                    device=torch.device("cpu"),
                    pretrained=fallback_path,
                )
            else:
                raise
        del model.visual
    ...
    def encode_with_transformer(self, text):
        ...
        # Check if the first block expects batch_first
        batch_first = False
        if hasattr(self.model.transformer.resblocks[0], 'attn'):
             if hasattr(self.model.transformer.resblocks[0].attn, 'batch_first'):
                 batch_first = self.model.transformer.resblocks[0].attn.batch_first
        
        if not batch_first:
            x = x.permute(1, 0, 2)  # NLD -> LND
        
        # Ensure attn_mask is on the correct device and dtype
        if self.model.attn_mask is not None:
            self.model.attn_mask = self.model.attn_mask.to(device=x.device, dtype=x.dtype)
            
        print(f"DEBUG: batch_first={batch_first}, x_in.shape={x.shape}")
        x = self.text_transformer_forward(x, attn_mask=self.model.attn_mask)
        if isinstance(x, dict) and "last" in x:
             print(f"DEBUG: x_out['last'].shape={x['last'].shape}")
        
        if self.legacy:
            o = x[self.layer]
            if batch_first:
                o = o.permute(1, 0, 2) # LND -> NLD
            o = self.model.ln_final(o)
            return o
        else:
            # Fix all keys if batch_first
            if batch_first:
                for k in ["last", "penultimate"]:
                    if k in x:
                        x[k] = x[k].permute(1, 0, 2)
            
            # Apply ln_final to "last" (and maybe others? original code only applied to "last" before pooling)
            # Original code:
            # o = x["last"]
            # o = self.model.ln_final(o)
            ...
            # We need to be careful. ln_final should be applied to "last" for pooling.
            # But does it apply to "penultimate"? Usually no.
...
class FrozenOpenCLIPImageEmbedder(AbstractEmbModel):
    ...
    def __init__(
        ...
        super().__init__()
        # Hack to use local weights if available
        if version == "laion2b_s39b_b160k" and arch == "ViT-bigG-14":
             local_path = "/home/liujiaxiang/MindAligner/models_weights/open_clip_pytorch_model.bin"
             if os.path.exists(local_path):
                 print(f"Loading local CLIP weights from {local_path}")
                 version = local_path
